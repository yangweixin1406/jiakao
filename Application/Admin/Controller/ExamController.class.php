...
public function daoru(){
     ...
     if($_POST){
       ...
       $classno=I("classno");
	    if($path==""){
		   $this->showmessage('请上传xls文件',U($this->urldir.$this->tablename.'/daoru/'),3);exit;
		}
		$arr=explode(".",$path);
	   ...
	   if(!is_file($path_full)){
	     $this->showmessage('文件不存在',U($this->urldir.$this->tablename.'/daoru/'),3);exit;
	   }
	  header("Content-type: text/html; charset=utf-8"); 
    ...
    // $allColumn = $currentSheet->getHighestColumn(); 
      /**取得一共有多少行*/
       ...
       for($currentRow =2;$currentRow<=$allRow;$currentRow++){//获取excel文件数据到数组
        ...
        $this->showmessage('已导入'.$success_count."条",U($this->urldir.$this->tablename.'/daoru/'),4);exit;
	}
