...
var conf={
	...
	action:"/phpsite/hc/kaoti/kaoti20170406/index.php/Home/Error/lists",
	app:"/phpsite/hc/kaoti/kaoti20170406/index.php",
	url:"/phpsite/hc/kaoti/kaoti20170406/index.php/Home/Error",
...
window.userid="<?php echo ($userinfo["id"]); ?>";
window.app_url="/phpsite/hc/kaoti/kaoti20170406/index.php";
window.g_actionurl="/phpsite/hc/kaoti/kaoti20170406/index.php/Home/Error/lists";
...
var cururl=String(window.location);
	  try{
		   window.uexOnload = function(type){
			  ...
			  if(cururl.indexOf("/home/user/login")!=-1){
...
$(function(){
	...
	$(".curnav .amenu,.curnav_a .amenu_a").click(function(){
		...
		$(caidanabs).css({width:$(window).width()+"px",height:$(window).height()+"px","overflow":"hidden","position":"absolute","left":"0px","top":"0px","z-index":"30000"});
		...
		str+="<li class='licontact'><a>联系我们</a></li>";
		str+="<li class='limyfavorites'><a href='"+conf.app+"/"+conf.module+"/favorites/lists"+"'>我的收藏</a></li>";
		str+="<li class='limyexamerror'><a href='"+conf.app+"/"+conf.module+"/error/lists"+"'>我的错题</a></li>";
		...
		//str+="<div style='position:absolute;bottom:0px;left:0px;width:100%;color:#1BBC9B;text-align:center;font-size:15px;'><div style='padding:10px; line-height:25px;'>联系电话<br>137000000<br/>联系扣扣<br/>632175205</div></div>";
		 ...
		 $(caidan_rel_con_inner).find(".licontact").click(function(){
...
<link rel="stylesheet" type="text/css" href="/phpsite/hc/kaoti/kaoti20170406/Public/exam/kaoti20170414.css?1">
</head>
<body>
...
<!--start-->
<div class="mybody">
<div class="curnav_a"><a  class="back_a" href="javascript:window.history.go(-1);"><em>d</em></a><center><span class="t"><?php echo ($mytitle); ?></span><span class="tishi tishi_title">已答0题/共<?php echo count($list);?>题</span></center><input type="button" class="bt_show_all_daan" value="显示答案"/><a class="amenu_a">≡</a></div>
<div style="height:50px; width:100%;"></div>
<div class="mymain">
        <div class="mymain_innner">
		<?php if(is_array($list)): foreach($list as $key=>$row): ?><div class="exam_item" id="exam_<?php echo ($key+1); ?>" data-value="<?php echo ($row["value"]); ?>" data-index="<?php echo ($key+1); ?>" data-title="<?php echo ($row["name"]); ?>" data-id="<?php echo ($row["id"]); ?>"  data-type="<?php echo ($row["type"]); ?>">
		   <div class="inner">
		    <h1 class="title"><?php echo ($key+1); ?>.<?php echo ($row["name"]); ?><span class="show_daan" style="display:none">(<?php echo ($row["value"]); ?>)</span></h1>
				<ul class="ul1">
				 <?php if(is_array($row["items"])): foreach($row["items"] as $key=>$option): ?><li>
					   <label>
						<?php if($row["type"] == 2): ?><input type="checkbox" name="radio_<?php echo ($row["id"]); ?>" value="<?php echo ($option["value"]); ?>" />
						 <?php else: ?> 
						<input type="radio" name="radio_<?php echo ($row["id"]); ?>" value="<?php echo ($option["value"]); ?>" /><?php endif; ?>
					   <?php echo ($option["lable"]); ?>、<?php echo ($option["title"]); ?>
					   </label>
					   </li><?php endforeach; endif; ?>
				</ul>
				<?php if($row[photo]!=""){ ?>
				  <div class="photo"><img src="/phpsite/hc/kaoti/kaoti20170406/<?php echo ($row["photo"]); ?>"/></div>
				<?php }?>
				<div class="clear"></div>
			   <div class="bt_qd_line"><input class="bt_add_favorites" type="button" value="加入我的收藏" title="加入我的收藏"/><input class="bt_remove_error" type="button" value="移除错题" title="移除错题"/><?php if($row["type"] == 2): ?><input type="button" name="bt_ok" class="bt_ok" value="<?php echo ($conf_lang[btcaption]); ?>" /><?php endif; ?></div>
			
		    <div class="op"><span class="my_daan">你的答案：<i>未做答</i></span><span class="data_daan"></span></div>
			</div>
		 </div><?php endforeach; endif; ?>
		<div class="divnull_a" style="display:<?php if(count($list)<1){echo '';}else{echo 'none';} ?>">没有错题记录的</div>
...
<script language="javascript" src="/phpsite/hc/kaoti/kaoti20170406/Public/exam/kaoti20170414.js"></script>
<script>
kaoti20170414.init({table:"<?php echo ($objtable); ?>",app:conf.app,module:conf.module,isadderror:0});
