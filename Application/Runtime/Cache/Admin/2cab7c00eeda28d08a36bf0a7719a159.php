...
.ul_info li.li2 textarea.txt1{ width:98%; height:100px;}
</style>
</head>
<body>
...
<div class="mytitle">
  <div class="con">
    <div class="left"><div class="t"><?php echo ($webtitle); ?></div><div class="b"><a href="/phpsite/hc/kaoti/kaoti20170406/index.php/Admin/Configex/lists/">返回列表</a></div></div><div class="right"></div>
  </div>
</div>
<!--标题结束-->
<!--主体开始-->
<div class="mycontent paddingtop80" >
<form id="form1" name="form1" method="post" action="<?php echo U('save');?>">
<div class="info1">
       <div class="tab"><ol class="tab_ol"><li class="cur">基本信息</li><li>其它信息</li><li>全部展开</li></ol></div>
	   <div class="tab_page">
       <ul class="ul_info"><li class="li1">考题表：</li><li class="li2"><input type="text"   name="objtable" id="objtable"   class="txt1" value="<?php echo ($info["objtable"]); ?>"/></li></ul>
	   <ul class="ul_info"><li class="li1">语言名称：</li><li class="li2"><input type="text"   name="title" id="title"   class="txt1" value="<?php echo ($info["title"]); ?>"/></li></ul>
	   <ul class="ul_info"><li class="li1">所用大类：</li><li class="li2">
						<select name="fatherno" id="fatherno">
						    <?php if(is_array($classlist)): foreach($classlist as $key=>$row): ?><option value="<?php echo ($row["classno"]); ?>"><?php echo ($row["title"]); ?></option><?php endforeach; endif; ?>
						</select>
	   </li></ul>
	        <ul class="ul_info"><li class="li1">阅读方式：</li><li class="li2">
	        ...
	        <ul class="ul_info"><li class="li1">开启状态：</li><li class="li2">
 ...
 <ul class="ul_info"><li class="li1">语言简称：</li><li class="li2"><input type="text"   name="lang" id="lang"   class="txt1" value="<?php echo ($info["lang"]); ?>"/></li></ul>
	  </div>
	   <div class="tab_page" style="display:none;">
	   <ul class="ul_info"><li class="li1">图片：</li><li class="li2">
	   <input type="hidden" name="photo" id="photo"  value="<?php echo ($info["photo"]); ?>"/><iframe src="/phpsite/hc/kaoti/kaoti20170406/Public/upload_20150420/upload/php/img_iframe.php?panel_id=my_show&input_only_id=photo&input_more_id=ImageList&maxnum=10&width=80&height=80" scrolling="no" frameborder="0" height="80" width="80" style="margin:0px;"></iframe>
	  </li></ul>
			...
			<ul class="ul_info"><li class="li1">副标题：</li><li class="li2"><input type="text"  name="title1" id="title1"   class="txt1" value="<?php echo ($info["title1"]); ?>"/></li></ul>
		    ...
		    <ul class="ul_info"><li class="li1">conf配置：</li><li class="li2"><textarea type="text"  name="conf" id="conf"   class="txt1"><?php echo ($info["conf"]); ?></textarea></li></ul>
		  <script> var id="<?php echo $id?>";if(id!=""){$("#ul_qturl").show();}</script>
	   </div>
	       <ul class="ul_info" id="ul_qturl" style="display:none;">
		     <li class="li1">前台url：</li>
		     <li class="li2">
						<a href="/phpsite/hc/kaoti/kaoti20170406/index.php?m=Qiyue&c=News&a=lists&classid=<?php echo ($info["id"]); ?>" target="_blank">/phpsite/hc/kaoti/kaoti20170406/index.php?m=Qiyue&c=News&a=lists&classid=<?php echo ($info["id"]); ?></a>
						<br/>
						<a href="/phpsite/hc/kaoti/kaoti20170406/index.php?m=Qiyue&c=about&a=lists&classid=<?php echo ($info["id"]); ?>" target="_blank">/phpsite/hc/kaoti/kaoti20170406/index.php?m=Qiyue&c=about&a=lists&classid=<?php echo ($info["id"]); ?></a>
			 </li>
		   </ul>
    </div>
  ...
  <div class="info_bts">
    <div class="con">
	 ...
	 <input type="button" onclick="javascript:window.location='/phpsite/hc/kaoti/kaoti20170406/index.php/Admin/Configex/lists/';"value="返回列表" class="bt" /><input type="submit" name="btsend" class="btsend" value="更新" /><input id="id" type="hidden" name="id" value="<?php echo ($info["id"]); ?>"/><input id="act" type="hidden" name="act" value="update"/> <input type="reset" value="重置" onclick="clearinput()" />
	<?php }else{ ?>
	
	<input type="button" onclick="javascript:window.location='/phpsite/hc/kaoti/kaoti20170406/index.php/Admin/Configex/lists/';"value="返回列表" class="bt" /> <input type="submit" name="btsend" value="确定" class="btsend" /><input id="act" type="hidden" name="act" value="add"/> <input type="reset" value="重置" onclick="clearinput()" />
      <?php }?>
    </div>
  </div>
...
$("#direction").val('<?php echo ($info["direction"]); ?>');

$("#fatherno option").each(function(){
   if($(this).val()=="<?php echo ($info["fatherno"]); ?>"){
      $(this)[0].selected=true;
   }
})
$("#direction option").each(function(){
   if($(this).val()=="<?php echo ($info["direction"]); ?>"){
      $(this)[0].selected=true;
   }
})
$("#status option").each(function(){
   if($(this).val()=="<?php echo ($info["status"]); ?>"){
      $(this)[0].selected=true;
   }
})
function check(){
