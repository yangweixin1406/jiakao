...
<div class="mytitle" style="z-index:100;">
	<div class="con">
		<div class="left"><div class="tt"><?php echo ($mytitle); ?>&nbsp;(<?php echo ($item["num"]); ?>/<?php echo ($item["rowcountz"]); ?>)</div><div class="bb"><a href11="<?php echo U('info');?>" style="color:#CCCCCC">添加订单</a></div><span class="btchazhao">查找</span></div>
		<div class="right">
			<form id="form1" name="form1" method="get" action="/phpsite/hc/kaoti/kaoti20170406/index.php/Admin/Orders/lists/" ><div class="l"></div>
			<div  class="r">
				<div class="row">
					 <span class="t">支付状态<em>：</em></span>
					 <span class="f">
						<select name="status" id="status" title="状态">
						<option value="">请选择</option>
						<?php if(is_array($statuslist)): foreach($statuslist as $key=>$row): ?><option value="<?php echo ($row["value"]); ?>"><?php echo ($row["title"]); ?></option><?php endforeach; endif; ?>
						</select>
					</span>
				</div>
				<div class="row">
				   <span class="t">姓名<em>：</em></span><span class="f"><input type="text" name="contact" id="contact" value="<?php echo ($item["contact"]); ?>" class="txt1s"/></span>
				</div>
				<div class="row">
				   <span class="t">订单编号<em>：</em></span><span class="f"><input type="text" name="orderno" id="orderno" value="<?php echo ($item["orderno"]); ?>" class="txt1s"/></span>
				</div>
				<div class="row">
				   <span class="t">交易ID<em>：</em></span><span class="f"><input type="text" name="id" id="id" value="<?php echo ($item["id"]); ?>"  class="txt1s"/></span>
				</div>
				<div class="row">
				   <span class="t" style="line-height:13px;">第三方订单号<em>：</em></span><span class="f"><input type="text" name="tradeno" id="tradeno" value="<?php echo ($item["tradeno"]); ?>"  class="txt1s"/></span>
				</div>
				<div class="row">
				   <span class="t">金额<em>：</em></span><span class="f"><input type="text" name="total" id="total" value="<?php echo ($item["total"]); ?>"  class="txt1s"/></span>
				</div>
				<div class="row">
               ...
               <table class="tablelist tableloglat" cellspacing="0">
				...
				<tbody style="text-align:center;">
					<?php if(is_array($list)): foreach($list as $key=>$value): ?><tr>
							...
							<td class="td1"><?php echo ($value["contact"]); ?></td>
							<td class="td2">订单编号:<?php echo ($value["logno"]); ?><br/>第三方订单号:<?php echo ($value["tradeno"]); ?></td>
							<td class="td3"><span class='ts_color'><?php echo ($value["total"]); ?>块</span><br/>支付方式:<?php echo ($value["payname"]); ?></td>
                            <td class="td4"><span class='status<?php echo ($value["status"]); ?>'><?php echo ($value["status_caption"]); ?></span></td>
							<td class="td5"><?php echo ($value["username"]); ?></td>
							<td class="td6">提交日期:<?php echo ($value["addtime"]); ?><br/>结束日期:<?php echo ($value["endtime"]); ?></td>
							<td class="tdop"><a href="/phpsite/hc/kaoti/kaoti20170406/index.php/Admin/Orders/info/id/<?php echo ($value["id"]); ?>">编辑</a>|<a href="/phpsite/hc/kaoti/kaoti20170406/index.php/Admin/Orders/delete/id/<?php echo ($value["id"]); ?>">删除</a></td>
