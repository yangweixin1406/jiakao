...
<form id="form1" name="form1" method="post" action="<?php echo U('reg');?>" onsubmit="return check();">
<div class="info1">


	   <div class="tab_page">
	   <fieldset><legend>会员注册配置</legend>
	    <ul class="ul_info"><li class="li1">配置开关：</li><li class="li2"><input type="hidden" id="reg_status" name="setting[reg][status]" value="<?php echo ($setting[reg][status]); ?>"   txt1="开"  txt2="关"/></li></ul>
		<ul class="ul_info"><li class="li1">是否激活：</li><li class="li2"><div ><input type="hidden" id="reg_state" name="setting[reg][state]" value="<?php echo ($setting[reg][state]); ?>"  txt1="是"  txt2="否"/></div><div style="width:100%; height:auto; clear:both;">就是注册完是否可用</div></li></ul>
		<ul class="ul_info"><li class="li1">有效期：</li><li class="li2"><input type="text" name="setting[reg][day]" value="<?php echo ($setting[reg][day]); ?>"  />天</li></ul>
		<ul class="ul_info"><li class="li1">注册验证方式：</li><li class="li2">
		   <label><input type="radio" class="codetype" name="setting[reg][codetype]" value="smscode" title="短信验证"/>短信验证</label>
		   <label><input type="radio" class="codetype" name="setting[reg][codetype]" value="emailcode" title="邮箱验证"/>邮箱验证</label>
		   <label><input type="radio" class="codetype" name="setting[reg][codetype]" value="code" title="普通验证码"/>普通验证码</label>
		   <label><input type="radio" class="codetype" name="setting[reg][codetype]" value="" title="不需要"/>不需要</label>
		  </li></ul>
	   </fieldset>
...
onoff("reg_status",100);
onoff("reg_state",100);
$(".codetype").each(function(){
   if($(this).val()=="<?php echo ($setting[reg][codetype]); ?>"){
