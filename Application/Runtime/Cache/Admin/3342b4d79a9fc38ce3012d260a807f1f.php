...
<?php if(is_array($configex)): foreach($configex as $key=>$row): ?><li class="li_a">
   <div class="tit"><?php echo ($row["lang"]); ?><span class="s">管理</span></div>
      <ul class="ul_b">
	    <li><a href="<?php echo U('Exam/lists',array('table'=>$row[objtable]));?>"><i class='ifa fa-bus'></i>考题列表</a></li>
	    <li><a href="<?php echo U('Exam/info',array('table'=>$row[objtable]));?>"><i class='ifa ifa fa-tags'></i>添加考题</a></li>
		<li><a href="<?php echo U('Exam/daoru',array('table'=>$row[objtable]));?>"><i class='ifa ifa fa-tags'></i>导入考题</a></li>
	</ul>
...
<div class="mytitle" style="z-index:100;">
	<div class="con">
		<div class="left"><div class="tt"><?php echo ($mytitle); ?>&nbsp;(<?php echo ($item["num"]); ?>/<?php echo ($item["rowcountz"]); ?>)</div><div class="bb"><a href="<?php echo U('exam/info');?>">发布考题</a></div><span class="btchazhao">查找</span></div>
		<div class="right">
			<form id="form1" name="form1" method="get" action="/phpsite/hc/kaoti/kaoti20170406/index.php/Admin/Exam/lists/" ><div class="l"></div>
			<div  class="r">
				<div class="row">
               ...
               <table class="tablelist tableloglat" cellspacing="0">
				<thead>
					<tr>
						...
						<th class='td3'>分类</th>
                        <th class='td4'>科目</th>
						<th class='td5'>录入日期</th>
						<th class='tdop'>操作</th>
					</tr>
				</thead>
				<tbody style="text-align:center;">
					<?php if(is_array($list)): foreach($list as $key=>$value): ?><tr>
                            <td class="td1"><input type="checkbox" name="ck" value="<?php echo ($value["id"]); ?>" /></td>
							<td class="td1"><?php echo ($value["name"]); ?></td>
							<td class="td2"><?php echo ($value["type_caption"]); ?></td>
							<td class="td3"><?php echo ($value["class_caption"]); ?></td>
                            <td class="td4"><?php echo ($value["kemu_caption"]); ?></td>
                            <td class="td5"><?php echo ($value["addtime"]); ?></td>
							<td class="tdop"><a href="/phpsite/hc/kaoti/kaoti20170406/index.php/Admin/Exam/info/id/<?php echo ($value["id"]); ?>">编辑</a>|<a href="/phpsite/hc/kaoti/kaoti20170406/index.php/Admin/Exam/delete/id/<?php echo ($value["id"]); ?>">删除</a></td>
...
<script>
$("#classno").val('<?php echo ($item["classno"]); ?>');
</script>
<script>
