...
class IndexController extends \Home\Controller\CommExamController{
    ...
    public function index(){
	  ...
	  $one=$mod->where($map)->find();  //find * from tb_setting where setting_key = kaoti;
	  ...
	  if($kaoti_setting["biaomoshi"]==2){
		 ...
		 $configex=M("configex")->where("1=1")->order("sort asc")->select();  //select * from tb_configex where id=1 order by asc;
		$this->assign("configex",$configex);
	
		$map1["fatherno"]=$this->fatherno;
		$class=M("class");
		$classlist=$class->where($map1)->select();  //select * form tb_class where fatherno = 00010004;
  ...
  public function check(){
        $user=M("user");
       $one =$user->where(array("UE_account"=>$_SESSION['uname']))->find();  //tb_user表
	   if(is_array($one)){
				$tgbz= M('tgbz');
				$where=array();
