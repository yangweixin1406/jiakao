...
mytxt=RequestValue(items,"mytxt")
TmpPath = "tmp/"&tmpid&"_"&index&".tmp"          '保存路径

'--------开始判断文件是否允许上传
Forum_upload="gif,jpg,png,bmp,jpeg,doc,docx,xlsx,xls,rar,zip,zp,mp4,mp3" '允许文件格式
...
'--------开始合并生成文件
 if(index=total)then
   ...
   if is_upload=true then 
		...
		file_path=big_src
		if(is_img) then
		    logo1="../../images/gz1.png"
		   'call JpgAddShuiYin(g_cengci&big_src,logo1,g_cengci&big_src)
		   if(mytxt<>"") then
		    call JpgAddText(g_cengci&big_src,mytxt,g_cengci&big_src)
		   end if
		end if
   else 
