...
<%
'这个文件跟ajax_file.asp的区别在于ajax_file图片不生成小图和中图，
...
'--------开始合并生成文件
 if(index=total)then
   redim fname(cint(total)),fstr(cint(total))
	...
	if(ext="bmp" or ext="jpg" or ext="png" or ext="gif") then '上传图片
	
			   if (g_dir1=g_dir2) and (g_dir2=g_dir3) then
						small_src=g_dir1&newname&"."&ext
						mid_src=g_dir2&newname&"."&ext&"."&ext
						big_src=g_dir3&newname&"."&ext&"."&ext&"."&ext
			   else
						small_src=g_dir1&newname&"."&ext
			  ...
			  if(g_is_small=0 and g_is_small=g_is_mid) then
			    big_src=g_dir3&newname&"."&ext
			  end if
			  if(g_is_small<>1)then
				small_src=big_src
			  end if
			  if(g_is_mid<>1)then 
				  mid_src=big_src
			  ...
			  big_fullpath=g_cengci&big_src '大图路径
			  mid_fullpath=g_cengci&mid_src '中图路径
			  small_fullpath=g_cengci&small_src '小图路径
			 ' call SaveBase64(big_fullpath,base64) '生成原图
			  call  SaveBytesToFile(fstr,big_fullpath)
			  if(g_is_small=1)then
			   call SavePhoto(big_fullpath,small_fullpath,g_w1) '生成小图
			  end if
			  if(g_is_mid=1)then
			   call SavePhoto(big_fullpath,mid_fullpath,g_w2) '生成中图
			  end if
			  status="success"
			  msg="后端程序图片上传成功"
			  file_path=big_src
   elseif is_upload=true then '上传其他文件类型
   ...
   response.Write("{""status"":"""&status&""",""error"":"""&error_str&""",""filepath"":"""&file_path&""",""small_src"":"""&small_src&""",""mid_src"":"""&mid_src&""",""big_src"":"""&big_src&""",""ext"":"""&ext&""",""cengci"":"""&g_cengci&""",""date"":"""&now()&""",""tmpid"":"""&tmpid&""",""title"":"""&title&"""}")
   response.End()
end if
'--------结束合并生成文件
