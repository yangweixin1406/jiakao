...
if($_POST["state"]=="myDaoCuTel"){
 ...
 //条件结束
   $mydb=new db();
	$rs=$mydb->myQuery("select fsTel from fs_user where fsId>0  ".$str." order by fsId desc ");
	$i=-1;
	//while开始
 while($row=mysql_fetch_array($rs)){
	$i=$i+1;	
	$objPHPExcel->getActiveSheet()->setCellValue('B'.($i+2), $row["fsTel"]);
...
$objWriter->save(str_replace('excel.php', $newfile, __FILE__));
echo '<a href="'.$newfile.'">下载</a>';
