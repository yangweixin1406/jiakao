...
for($i=0;$i<count($title);$i++){
 ...
 $mydb=new db();
	$rs=$mydb->myQuery("select * from fs_card where fsZu=".$fsZu);
	$i=-1;
	   while($row=mysql_fetch_array($rs)){


       // $arr[$i]=array($i,$row["fsIdNo"],$row["fsPwd"],$row["fsFaceValue"]);
		$i=$i+1;
...
$objPHPExcel->getActiveSheet()->setCellValue('B'.($i+2), $i+1);
$objPHPExcel->getActiveSheet()->getStyle('C'.($i+2))->getNumberFormat()->setFormatCode("@");
$objPHPExcel->getActiveSheet()->setCellValue('C'.($i+2), " ".strval($row["fsIdNo"]." "));

$objPHPExcel->getActiveSheet()->setCellValue('D'.($i+2), " ".$row["fsPwd"]." ");
$objPHPExcel->getActiveSheet()->setCellValue('E'.($i+2),$row["fsFaceValue"]);
...
$newname=date("YmdHis").'.xls';
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
$objWriter->save(str_replace('excel.php', $newname, __FILE__));
echo '<a href="'.$newname.'">'.iconv('utf-8','gbk','下载excel.xls').'</a><a href="../../fs_admin/fs_cardList.php">'.iconv('utf-8','gbk','返回列表').'</a>';
?>
