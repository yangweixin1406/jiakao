...
dsy.add("0_33_0",["中正区","大同区","中山区","松山区","大安区","万华区","信义区","士林区","北投区","内湖区","南港区","文山区","其它区"]);
dsy.add("0_33_1",["新兴区","前金区","芩雅区","盐埕区","鼓山区","旗津区","前镇区","三民区","左营区","楠梓区","小港区","其它区"]);
dsy.add("0_33_2",["中西区","东区","南区","北区","安平区","安南区","其它区"]);
dsy.add("0_33_3",["中区","东区","南区","西区","北区","北屯区","西屯区","南屯区","其它区"]);
dsy.add("0_33_4",["金门县"]);
dsy.add("0_33_5",["南投县"]);
dsy.add("0_33_6",["仁爱区","信义区","中正区","中山区","安乐区","暖暖区","七堵区","其它区"]);
dsy.add("0_33_7",["东区","北区","香山区","其它区"]);
dsy.add("0_33_8",["东区","西区","其它区"]);
dsy.add("0_33_9",["新北市"]);
dsy.add("0_33_10",["宜兰县"]);
dsy.add("0_33_11",["新竹县"]);
dsy.add("0_33_12",["桃园县"]);
...
dsy.add("0_33_19",["花莲县"]);
dsy.add("0_33_20",["澎湖县"]);
dsy.add("0_33",["台北市","高雄市","台南市","台中市","金门","南投","基隆市","新竹市","嘉义市","新北","宜兰","新竹","桃园","苗栗","彰化","嘉义","云林","屏东","台东","花莲","澎湖"]);
var s=["fsProvince","fsCity","fsCountry"];//三个select的name
var opt0 = ["省份","城市","县/区"];//初始值
...
var s=["fsProvince","fsCity","fsCountry"];//三个select的name
var opt0 = ["省份","城市","县/区"];//初始值
function init_area_a(opt){  //初始化函数
   var opt_arr = ["省份","城市","县/区"];//初始值
   if(typeof(opt.null_arr)!="undefined"){
	     opt_arr = opt.null_arr;
   }
   var s_arr=[opt.province,opt.city,opt.country];//三个select的name 
   var change_a=function(v){
			var str="0";
			for(i=0;i<v;i++){
				str+=("_"+(document.getElementById(s_arr[i]).selectedIndex-1));
			};
			var ss=document.getElementById(s_arr[v]);
			with(ss){
				length = 0;
				options[0]=new Option(opt_arr[v],opt_arr[v]);
				if(v && document.getElementById(s_arr[v-1]).selectedIndex>0 || !v){
					if(dsy.Exists(str)){
						ar = dsy.Items[str];
						for(i=0;i<ar.length;i++){
							options[length]=new Option(ar[i],ar[i]);
				...
				if(++v<s_arr.length){change_a(v);}
			}//End with
   }
	for(i=0;i<s_arr.length-1;i++){
		var sel=document.getElementById(s_arr[i]);
		sel.setAttribute("myindex",(i+1));
		document.getElementById(s_arr[i]).onchange=function(){
			    change_a(this.getAttribute("myindex"));
		}
	  // document.getElementById(s_arr[i]).onchange=new Function("change_a("+(i+1)+")");
	}
	change_a(0);
	var set_select_value=function(op){
			var obj=document.getElementById(op.id);
			var len=obj.options.length;
			for(var j=0;j<len;j++){
				if(obj.options[j].value==op.value){
					obj.selectedIndex=j;
				}
			}
	}
	var obj1=document.getElementById(s_arr[0]);
	var obj2=document.getElementById(s_arr[1]);
	var obj3=document.getElementById(s_arr[2]);
	var datavalue1=obj1.getAttribute("data-value"),datavalue2=obj2.getAttribute("data-value"),datavalue3=obj3.getAttribute("data-value");
	if(typeof(datavalue1)!="undefined"){ set_select_value({id:s_arr[0],value:datavalue1});change_a(1);}
	if(typeof(datavalue2)!="undefined"){ set_select_value({id:s_arr[1],value:datavalue2});change_a(2);}
	if(typeof(datavalue3)!="undefined"){ set_select_value({id:s_arr[2],value:datavalue3});}
	
	if(typeof(opt.value1)!="undefined"){ set_select_value({id:s_arr[0],value:opt.value1});change_a(1);}
	if(typeof(opt.value2)!="undefined"){set_select_value({id:s_arr[1],value:opt.value2});change_a(2);}
	if(typeof(opt.value3)!="undefined"){set_select_value({id:s_arr[2],value:opt.value3});}
}
