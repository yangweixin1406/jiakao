...
<script src="js/20150401.js?13"></script>
</head>
<body>
<div id="item1" style=" width:100%; background-color:#eeeeee;position:relative;overflow:visible;display:none;">
<form  name="upform1" id="item1_upform1" enctype="multipart/form-data" method="post" action="img_upload.php"  style="margin:0px; padding:0px; line-height:0px; border:0px;">
...
var arr=cururl.split("img_iframe.php");
var cur_path=arr[0];
...
if(arr.length>1){
	...
	item1_divbg.innerHTML="<table cellpadding='0' cellspacing='0' border='0' style='height:"+window.clientHeight+"px;width:100%;'><tr style='height:100%;'><td valign='middle' style='height:100%;'><img src='"+g_fengmian_src+"' style='width:"+width+"px; height:"+height+";'/></td></tr></table>";
	  ...
	  if(us.indexOf("MSIE ")!=-1&&us.indexOf("MSIE 10.0")==-1){
		  // if(us.indexOf("MSIE 8.0")!=-1||us.indexOf("MSIE 6.0")!=-1||us.indexOf("MSIE 7.0")!=-1||us.indexOf("MSIE 9.0")!=-1){
			...
			for(var i=0;i<16;i++){
				...
				Ul.Init1({id:"mydiv",dir1:g_small_dir1,dir2:g_mid_dir2,dir3:g_big_dir3,objtable:objtable,input:inputdan,inputmore:inputmore,width:width,height:window.height,objpanel:objpanel,maxnum:maxnum,float:float,imgstr:smallsrc});
	  }else{ 
...
//		 document.getElementById("item1_btsend").click();
         item2.style.display="";
		 item2.style.width=width+"px";
		 item2.style.height=height+"px";
		 var Ul=Upload();//显示初始图片
		 Ul.Init({id:"mydiv",dir1:g_small_dir1,dir2:g_mid_dir2,dir3:g_big_dir3,objtable:objtable,input:inputdan,inputmore:inputmore,width:width,height:height,objpanel:objpanel,maxnum:maxnum,float:float,imgstr:smallsrc});
	}
